...
import { Star, ShoppingCart, Plus, Minus, X } from 'lucide-react'
    ...
    try {
      ...
      if (res.ok && response.success) {
        const transformedProducts = response.data.map((product: {
          ...
          description: product.description || 'Fresh and high-quality product',
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              <h1 className="text-2xl font-bold text-orange-600">Fresh Vegetables</h1>
              <p className="ml-4 text-gray-600">Crisp and nutritious selection</p>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-4">Farm-Fresh Vegetables</h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Straight from local farms, our vegetables deliver maximum freshness and nutritional value to your table.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map((product) => {
            const buttonClass = isInCart(product.id)
              ? 'bg-gray-200 text-gray-600 cursor-not-allowed'
              : product.inStock
              ? 'bg-orange-600 text-white hover:bg-orange-700'
              : 'bg-gray-200 text-gray-400 cursor-not-allowed'
            
            const buttonText = isInCart(product.id)
              ? 'In Cart'
              : product.inStock
              ? 'Add to Cart'
              : 'Out of Stock'
            ...
            return (
              <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative">
                  ...
                  <div className="absolute top-2 left-2 bg-orange-600 text-white px-2 py-1 rounded text-xs">
                    Vegetable
                  </div>
                </div>
                <div className="p-4">
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No vegetables found matching your criteria.</p>
          </div>
        )}
      </main>
                    ...
                    <div key={item.product.id} className="bg-gray-50 rounded-xl p-4 hover:bg-gray-100 transition-colors">
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4">
                            <span className="text-lg font-bold text-orange-600">Rs.{item.product.price}</span>
            ...
            {cartItems.length > 0 && (
              <div className="border-t border-gray-100 p-6 bg-gray-50 rounded-b-2xl">
                <div className="space-y-3 mb-6">
                  ...
                  <div className="border-t pt-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold text-orange-600">Rs.{getCartTotal()}</span>
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button className="bg-orange-600 text-white py-3 rounded-lg font-medium hover:bg-orange-700 transition-colors shadow-lg">
                    Proceed to Checkout
                  </button>
                </div>
              </div>
